<script lang="ts">
	...
	import CreateVersionForm from '$lib/components/CreateVersionForm.svelte';
	...
	// Create New Version modal state
		...
		if (versions.length > 0) {
			...
			if (searchQuery.trim()) {
				...
				filteredVersions = versions.filter(
					(v) =>
						v.versionNumber.toString().includes(query) || v.createdAt.toLowerCase().includes(query)
				);
			} else {
...
<div class="page-container">
	...
	{:else if project}
		...
		<div class="versions-section">
			...
			{:else if versions.length === 0}
				<div class="empty-versions">
					...
					{#if isAdminOrOwner}
						<button class="btn-create" on:click={createNewVersion}> Create First Version </button>
					{/if}
				</div>
			{:else}
								...
								<tr>
									...
									<td class="center-aligned">
										<div class="version-actions">
											<a
												href={`/projects/${projectId}/versions/${version.versionNumber}`}
		...
		<CreateVersionForm
			...
			on:close={() => (showCreateVersionModal = false)}
			on:versionCreated={() => {
				// Will be called when a version is created, even if user stays in modal
				loadVersions();
			}}
			on:refreshProject={() => {
				loadProject();
